...
def get_sentence_scores(hyp, ref, src, n=4, num_iterations=500):
    ...
    val = [g for g in gleu_calculator.run_iterations(num_iterations=num_iterations,
                                                             source=src,
                                                             hypothesis=hyp,
                                                             per_sent=True)]
    
    return [float(x[0])*100 for x in val]





def get_corpus_level_gleu(hyp, ref, src, n=4, num_iterations=500):
    """get corpus-level gleu scores"""
    gleu_calculator = GLEU(n)
    gleu_calculator.load_sources(src)
    gleu_calculator.load_references([ref])
    val = float([g for g in gleu_calculator.run_iterations(num_iterations=num_iterations,
                                                             source=src,
                                                             hypothesis=hyp,
                                                             per_sent=False)][0][0])*100
    return val
...
if __name__ == '__main__':
    ...
    for hpath in args.hyp:
        ...
        if len(args.ref) == 1:
            ...
            if args.sent:
                print(get_sentence_scores(hpath, args.ref[0], args.src, n=args.n, num_iterations=args.iter))
            else:
                print(get_corpus_level_gleu(hpath, args.ref[0], args.src, n=args.n, num_iterations=args.iter))
...
#         if len(args.ref) == 1:
#             print("There is one reference. NOTE: GLEU is not computing the confidence interval.")
#             val = [g for g in gleu_calculator.run_iterations(num_iterations=args.iter,
#                                                              source=args.src,
#                                                              hypothesis=hpath,
#                                                              per_sent=args.sent)]
#             scores = float(val[0][0])*100
#             # print([g for g in gleu_calculator.run_iterations(num_iterations=args.iter,
#             #                                                  source=args.src,
#             #                                                  hypothesis=hpath,
#             #                                                  per_sent=args.sent)][0][0])
#             print(scores)
#             print(*[float(x[0])*100 for x in val])
#         else:
#             print([g for g in gleu_calculator.run_iterations(num_iterations=args.iter,
#                                                              source=args.src,
#                                                              hypothesis=hpath,
